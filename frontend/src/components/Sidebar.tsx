...
export default function Sidebar({ onCapture }: SidebarProps) {
  ...
  const [config, setConfig] = useState({ iso: "100", aperture: "5.6", shutterspeed: "1/100" });
    ...
    <div className="w-64 panel border-r flex flex-col p-4 shadow-xl z-10 transition-all">
      <div className="flex items-center gap-2 mb-8">
        ...
        <span className="text-[10px] uppercase tracking-widest font-bold opacity-60">
          Canon EOS 1100D {status.connected ? "Connected" : "Disconnected"}
        </span>
      </div>
      ...
      <div className="space-y-6">
        ...
        <section>
          ...
          <div className="space-y-4">
            ...
            <div className="space-y-1">
              <label className="text-[10px] opacity-40 uppercase">Aperture</label>
              <select 
                value={config.aperture}
                onChange={(e) => handleConfigChange("aperture", e.target.value)}
                className="w-full dark-input h-8 text-[11px] bg-black/40 border-white/5"
              >
