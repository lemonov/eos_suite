...
export default function Home() {
  const [activeTab, setActiveTab] = useState<"library" | "stacker" | "canvas">("library");
  ...
  return (
    <CanvasProvider>
      <main className="flex h-screen w-screen overflow-hidden bg-dark-bg text-dark-text">
        ...
        {/* Main Content Area */}
        <div className="flex flex-col flex-1 border-x border-dark-border">
          {/* Top Navigation / Module Switcher */}
          <div className="flex items-center justify-center h-10 border-b border-dark-border bg-dark-sidebar/50">
            <button
              onClick={() => setActiveTab("library")}
              className={cn(
                "flex items-center gap-2 px-4 h-full text-[10px] uppercase tracking-wider font-semibold transition-colors",
                activeTab === "library" ? "text-dark-accent border-b-2 border-dark-accent" : "text-white/40 hover:text-white/60"
              )}
            >
              <ImageIcon size={14} /> Lighttable
            </button>
            <button
          ...
          {/* Viewport */}
          <div id="center-viewport" className="flex-1 overflow-auto relative bg-dark-bg">
